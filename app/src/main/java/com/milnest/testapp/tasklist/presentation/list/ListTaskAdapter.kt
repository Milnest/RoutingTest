...
class ListTaskAdapter(val cbClickListener: CbClickListener):
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (viewType){
            CHECKBOX_ITEM_TYPE -> {
                CbHolder(LayoutInflater.from(parent.context).inflate(R.layout.task_list_item_list_cb,
                        parent, false))
            }
            else /*ADD_ITEM_TYPE*/ -> {
                AddHolder(LayoutInflater.from(parent.context).inflate(R.layout.task_list_item_list_add_cb,
                        parent, false))
            }
        }
